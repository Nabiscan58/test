...
function GetCauseOffDeath()
    local DeathReason = "est morte de chute"
    if IsEntityDead(PlayerPedId()) then
		...
		elseif (Killer == nil) then
			...
			elseif IsBomb(DeathCauseHash) then
				DeathReason = "est morte ~r~brûlée au troisième degré"
            ...
            for seat = -1, maxPassengers - 1 do
                ...
                if passenger ~= 0 then -- Check if the seat is not empty
                    ...
                    if pedType ~= 28 then -- 28 is for animals
                        if seat == -1 then 
                            DeathReason = "est morte écrasée par un véhicule"
                        end
                    end
                end
            end
