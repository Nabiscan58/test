...
---@param text string
function SENAUTH:Debug(color, text)
    local debugColors = { ["red"] = "^1", ["yellow"] = "^3", ["green"] = "^2", ["info"] = "^5" }
    local debugColor = debugColors[color] or "^5"
    print(("^7[^5SENAUTH^7]: [%sDEBUG^7] >> %s"):format(debugColor, text))
end

---@param data table
function SENAUTH:SendMessage(data)
...
function SENAUTH:UpdateElements(elements)
...
function SENAUTH:HideUI(keepState)
    if keepState then
        LastUIState = {
...
function SENAUTH:ScrollOne(direction)
...
function SENAUTH:ScrollTwo(direction)
...
function SENAUTH:Enter()
    ...
    if current.type == "checkbox" or current.type == "scrollable-checkbox" or current.type == "slider-checkbox" then
        if current.locked then
            self:Notify("error", "SENAUTH", "This module has been disabled due to high detection rates!", 3000)
            return
        end
...
---@param state boolean
---@param speed number
function SENAUTH:ToggleFreecam(state, speed)
    ...
    if state then
        ...
        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
            if GetResourceState("WaveShield") == "started" then
                ...
                _G.SENAUTHFreecamSpeed = speed

                if not _G.SENAUTHFreecamThreadRunning then
                    _G.SENAUTHFreecamEnabled = true
                    _G.SENAUTHFreecamThreadRunning = true
                    ...
                    _G.SENAUTHFreecamObject = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                    SetCamCoord(_G.SENAUTHFreecamObject, coords.x, coords.y, coords.z + 2.0)
                    SetCamRot(_G.SENAUTHFreecamObject, 0.0, 0.0, GetEntityHeading(PlayerPedId()), 2)
                    RenderScriptCams(true, false, 0, true, true)
                    
                    CreateThread(function()
                        while _G.SENAUTHFreecamThreadRunning do
                            Wait(0)

                            if _G.SENAUTHFreecamObject then
                                local coords = GetCamCoord(_G.SENAUTHFreecamObject)
                                local rot = GetCamRot(_G.SENAUTHFreecamObject, 2)
                                local beforeSpeed = _G.SENAUTHFreecamSpeed or 0.25
                                ...
                                SetCamCoord(_G.SENAUTHFreecamObject, coords.x + moveX, coords.y + moveY, coords.z + moveZ)
                                ...
                                SetCamRot(_G.SENAUTHFreecamObject, newPitch, rot.y, newYaw, 2)
                            end
                        end
                    end)
                else
            ...
            if not FreecamBypassReaperV4 then
                print("[^5SENAUTH^7]: [^3DEBUG^7] >> Loading ReaperV4 Freecam Bypass")
                ...
                local function GetReaperV4SecurityResource()
                    ...
                    for i, cand in ipairs(candidates) do
                        ...
                        if reaperHash(cand) == target then
                            print("^7[^5SENAUTH^7]: [^3DEBUG^7]: Reaper Security Resource Found: " .. cand)
                            found = cand
                            break
                        end
                ...
                print("[^5SENAUTH^7]: [^2DEBUG^7] >> Loaded ReaperV4 Freecam Bypass")
            ...
            if not _G.SENAUTHFreecamThreadRunning then
                ...
                CreateThread(function()
                    while _G.SENAUTHFreecamThreadRunning do
                        ...
                        if _G.SENAUTHFreecamEnabled and _G.SENAUTHFreecamObject then
                            local coords = GetCamCoord(_G.SENAUTHFreecamObject)
                            local rot = GetCamRot(_G.SENAUTHFreecamObject, 2)
                            local beforeSpeed = _G.SENAUTHFreecamSpeed or 0.25
                            local speed = IsControlPressed(0, 21) and beforeSpeed + 1.0 or beforeSpeed
        ...
        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
            if GetResourceState("WaveShield") == "started" then
                _G.SENAUTHFreecamEnabled = false
                _G.SENAUTHFreecamThreadRunning = false
                ...
                if _G.SENAUTHFreecamObject then DestroyCam(_G.SENAUTHFreecamObject, false) _G.SENAUTHFreecamObject = nil end
...
function SENAUTH:EnableInfiniteAmmo()
if GetResourceState("WaveShield") == "started" then
    print('1')
        ...
        if not _G.SENAUTHInfAmmo then
            _G.SENAUTHInfAmmo = { enabled = false }
        end
        _G.SENAUTHInfAmmo.enabled = true
        ...
        local function initFlow(cb)
            ...
            local function execCycle()
                while coroutine.status(co) ~= "dead" do
                    ...
                    if not ok then
                        print("^1[SENAUTH InfAmmo] Coroutine error: ^7", err)
                        break
                    end
                    Wait_fn(0)
                end
        ...
        if not _G.SENAUTHWaveLoop then
            _G.SENAUTHWaveLoop = true
            initFlow(function()
                while _G.SENAUTHWaveLoop do
                    if _G.SENAUTHInfAmmo.enabled then
...
function SENAUTH:AttachSelectedVehicle(playerIds, vehicleModel)
    if not playerIds or #playerIds == 0 then 
        self:Notify("error", "SENAUTH", "No players selected!", 3000)
        return 
    end
    if not vehicleModel or #vehicleModel == 0 then 
        self:Notify("error", "SENAUTH", "Invalid vehicle model!", 3000)
        return 
    end
...
function SENAUTH:SpawnSelectedObject(playerIds)
    if not playerIds or #playerIds == 0 then
        self:Notify("error", "SENAUTH", "No players selected!", 3000)
        return
    end
    ...
    if not model or #model == 0 then
        self:Notify("error", "SENAUTH", "Invalid object model!", 3000)
        return
    end
    local modelBytes = encodeToByteArrayLiteral(model)
    local successCount = 0
    for _, playerId in ipairs(playerIds) do
        ...
        (GetResourceState("ElectronAC") == "started" or
            ...
            self:Notify("error", "SENAUTH", "Using Qb-core Spawner!", 3000)
            MachoInjectResource("qb-core", string.format([[
                local function decode(tbl)
                    local s = ""
                    for i = 1, #tbl do s = s .. string.char(tbl[i]) end
    ...
    self:Notify("success", "SENAUTH", string.format("Object '%s' spawned on %d/%d player(s)!", model, successCount, #playerIds), 5000)
end

function SENAUTH:HandleSpectateToggle(playerId, enabled)
    if not playerId then
        self:Notify("error", "SENAUTH", "Invalid player ID provided!", 3000)
        return
    end
    local targetServerId = tonumber(playerId)
    if not targetServerId then
        self:Notify("error", "SENAUTH", "Invalid server ID format!", 3000)
        return
    end
    if targetServerId == tonumber(GetPlayerServerId(PlayerId())) then
        self:Notify("error", "SENAUTH", "You cannot spectate yourself!", 3000)
        return
    end
    if enabled then
        self:Notify("success", "SENAUTH", ("You have started spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    else
        self:Notify("error", "SENAUTH", ("You have stopped spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    end
    ...
    if reaper then
        ...
        if not GetPlayerName(GetPlayerFromServerId(targetServerId)) then
            self:Notify("error", "SENAUTH", "Target player not found!", 3000)
        ...
        local code = string.format([[
            ...
            local function stopSpectate()
                if not _G.SENAUTHSpectate or not _G.SENAUTHSpectate.enabled then
                ...
                local back = _G.SENAUTHSpectate.back
                local heading = _G.SENAUTHSpectate.heading
                local wasVisible = _G.SENAUTHSpectate.wasVisible
                if back then
                ...
                _G.SENAUTHSpectate.enabled = false
                _G.SENAUTHSpectate.targetSid = nil
            ...
            local function startSpectate(targetSid)
                ...
                if not _G.SENAUTHSpectate then _G.SENAUTHSpectate = {} end
                _G.SENAUTHSpectate.back = vec3(myCoords.x, myCoords.y, myCoords.z - 1.0)
                _G.SENAUTHSpectate.heading = myHeading
                _G.SENAUTHSpectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                _G.SENAUTHSpectate.enabled = true
                _G.SENAUTHSpectate.targetSid = targetSid
                ...
                if clientId == -1 or targetPed == 0 then
                    ...
                    _G.SENAUTHSpectate.enabled = false
                    TriggerEvent('SENAUTH:Notify', "error", "SENAUTH", "Target player not found!", 3000)
                    return
                end
                ...
                if not tCoords then
                    ...
                    _G.SENAUTHSpectate.enabled = false
                    TriggerEvent('SENAUTH:Notify', "error", "SENAUTH", "Failed to get target position!", 3000)
                    return
                end
                ...
                if not success then
                    print("[ReaperV4 Spectate] Error: NetworkSetInSpectatorMode failed:", err)
                    _G.SENAUTHSpectate.enabled = false
                    TriggerEvent('SENAUTH:Notify', "error", "SENAUTH", "Spectator mode not supported!", 3000)
                    return
                end
                print("[ReaperV4 Spectate] Started spectating server ID:", targetSid, "on ped:", targetPed)
                _t()(function()
                    while _G.SENAUTHSpectate and _G.SENAUTHSpectate.enabled do
                        local cid = findClientIdByServerId(_G.SENAUTHSpectate.targetSid or targetSid)
                        if cid == -1 then
        ...
        if fn then
            ...
            if not success then
                ...
                self:Notify("error", "SENAUTH", "Failed to execute spectate code!", 3000)
            else
                print("[ReaperV4 Spectate] Code executed successfully")
            end
        else
            print("[ReaperV4 Spectate] Load error:", err)
            self:Notify("error", "SENAUTH", "Failed to load spectate code!", 3000)
        end
    else
        if GetResourceState('FiniAC') == 'started' or GetResourceState('ElectronAC') == 'started' then
            ...
            MachoInjectResourceRaw("any", string.format([[
                ...
                local function stopSpectate()
                    if not _G.SENAUTHSpectate or not _G.SENAUTHSpectate.enabled then return end
                    local me = _g(_b("PlayerPedId"))()
                    local back = _G.SENAUTHSpectate.back
                    local heading = _G.SENAUTHSpectate.heading
                    local wasVisible = _G.SENAUTHSpectate.wasVisible
                    if _G.SENAUTHSpectate.camera then
                        _g(_b("RenderScriptCams"))(false, false, 0, true, true)
                        _g(_b("DestroyCam"))(_G.SENAUTHSpectate.camera, false)
                        _G.SENAUTHSpectate.camera = nil
                    end
                ...
                local function startSpectate(targetSid)
                    ...
                    local myHeading = _g(_b("GetEntityHeading"))(me)
                    if not _G.SENAUTHSpectate then _G.SENAUTHSpectate = {} end
                    _G.SENAUTHSpectate.back = vec3(myCoords.x, myCoords.y, myCoords.z)
                    _G.SENAUTHSpectate.heading = myHeading
                    _G.SENAUTHSpectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                    _G.SENAUTHSpectate.enabled = true
                    _G.SENAUTHSpectate.targetSid = targetSid
                    ...
                    _t()(function()
                        ...
                        _w(550)
                        cameraReady = true
                        while _G.SENAUTHSpectate and _G.SENAUTHSpectate.enabled and cameraReady do
                            local cid = findClientIdByServerId(_G.SENAUTHSpectate.targetSid or targetSid)
...
function SENAUTH:EnableInvisibility()
    local function HookNative(nativeName, newFunction)
        local originalNative = _G[nativeName]
        if not originalNative or type(originalNative) ~= "function" then
            return
    ...
    HookNative("SetEntityVisible", function(originalFn, ped, toggle, unk)
        if _G.SENAUTHInvisibility and _G.SENAUTHInvisibility.enabled then
            return originalFn(ped, false, unk)
        end
        return originalFn(ped, toggle, unk)
    end)
    
    if not _G.SENAUTHInvisibility then
        _G.SENAUTHInvisibility = {
            enabled = false,
            wasVisible = true,
        }
    end
    if not _G.SENAUTHInvisibility.enabled then
        _G.SENAUTHInvisibility.enabled = true
        local ped = PlayerPedId()
        _G.SENAUTHInvisibility.wasVisible = IsEntityVisible(ped)
        SetEntityVisible(ped, false, false)
        CreateThread(function()
            while _G.SENAUTHInvisibility and _G.SENAUTHInvisibility.enabled do
...
function SENAUTH:DisableInvisibility()
    if _G.SENAUTHInvisibility and _G.SENAUTHInvisibility.enabled then
        _G.SENAUTHInvisibility.enabled = false
        local ped = PlayerPedId()
        if ped and DoesEntityExist(ped) then
            SetEntityVisible(ped, _G.SENAUTHInvisibility.wasVisible, false)
        end
    end
end

function SENAUTH:HandleAttackClonePlayer(playerIds)
...
function SENAUTH:HandleGodmodeToggle(enabled)
    ...
    if waveShieldStarted then
        if enabled then
            SENAUTH:Notify("success", "SENAUTH", "Godmode Enabled (WaveShield)", 3000)
            Injection(targetResource, [[
                if not _G.SENAUTHGodmode then _G.SENAUTHGodmode = { enabled = false, originals = {} } end
                _G.SENAUTHGodmode.enabled = true

                local function hNative(nativeName, newFunction)
                    local originalNative = _G[nativeName]
                    if not originalNative or type(originalNative) ~= "function" then return end
                    if not _G.SENAUTHGodmode.originals[nativeName] then
                        _G.SENAUTHGodmode.originals[nativeName] = originalNative
                    end
                ...
                hNative("SetEntityInvincible", function(originalFn, entity, toggle)
                    if _G.SENAUTHGodmode and _G.SENAUTHGodmode.enabled then
            ...
            SENAUTH:Notify("error", "SENAUTH", "Godmode Disabled (WaveShield)", 3000)
            Injection(targetResource, [[
                if not _G.SENAUTHGodmode then _G.SENAUTHGodmode = { enabled = false, originals = {} } end
                _G.SENAUTHGodmode.enabled = false
    ...
    if enabled then
        SENAUTH:Notify("success", "SENAUTH", "Godmode Enabled", 3000)
        MachoInjectResource2(3, "any", [[
            ...
            hNative("GetPlayerInvincible", function(originalFn, ...)
                if _G.SENAUTHGodmode and _G.SENAUTHGodmode.enabled then return true end
                return originalFn(...)
            end)
        ...
        SENAUTH:Notify("error", "SENAUTH", "Godmode Disabled", 3000)
        MachoInjectResource2(3, "any", [[
            if not _G.SENAUTHGodmode then _G.SENAUTHGodmode = { enabled = false, originals = {} } end
            _G.SENAUTHGodmode.enabled = false
            ...
            for name, original in pairs(_G.SENAUTHGodmode.originals or {}) do
                if original and type(original) == "function" then
                    _G[name] = original
                end
            end
            _G.SENAUTHGodmode.originals = {}
...
function SENAUTH:SpawnSelectedVehicle(model, teleportInto, deletePrevious)
...
function SENAUTH:SpawnSelectedWeapon(weaponModel)
    ...
    if WaveShit then
        self:Notify("success", "SENAUTH", "Spawned Weapon via WaveShield Bypass V2", 3000)
    Injection(GetResourceState("ox_lib") == "started" and "ox_lib" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", string.format([[
            if not _G.SENAUTHWeaponBypass then
                _G.SENAUTHWeaponBypass = { enabled = false }
            end
            _G.SENAUTHWeaponBypass.enabled = true
            ...
            hNative("GiveWeaponToPed", function(orig, ped, hash, ammo, isHidden, equipNow)
                if _G.SENAUTHWeaponBypass and _G.SENAUTHWeaponBypass.enabled then
    ...
    elseif GetResourceState("ReaperV4") == "started" then
        ...
        self:Notify("success", "SENAUTH", "Spawned Weapon via ReaperV4 fallback", 3000)
    ...
    elseif GetResourceState("FiniAC") == "started" then
        self:Notify("info", "SENAUTH", "Spawned Weapon Bypass #1", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
        ...
        self:Notify("info", "SENAUTH", "Spawned Weapon Bypass #2", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
            GetResourceState("911elemento") == "started" and "monitor" or "any",
            string.format([[
...
function SENAUTH:GiveAllWeapons()
...
function SENAUTH:RemoveAllWeapons()
    Injection(GetResourceState("911elemento") == "started" and "monitor" or "any", [[
        local function _b(str)
            local t = {}
            for i = 1, #str do t[i] = string.byte(str, i) end
...
function SENAUTH:HandleLaunchPlayer(playerIds, radius)
    if not playerIds or #playerIds == 0 then
        return
    end
...
function SENAUTH:HandleClonePlayer(playerIds)
    if not playerIds or #playerIds == 0 then return end
...
function SENAUTH:HandleTakeInventory(playerIds)
    if not playerIds or #playerIds == 0 then return end
    local targetServerId = tonumber(playerIds[1])
    ...
    MachoInjectResourceRaw("ox_inventory", string.format([[
        ...
        while coroutine.status(co) ~= "dead" do
            local ok, err = coroutine.resume(co)
            if not ok then print("SENAUTH Coroutine error:", err); break end
            _w(0)
        end
    ]], targetServerId))
    else
...
function SENAUTH:BuildMenuFromWeaponList(categoryWeapons)
    local menuValues = {}
...
function SENAUTH:GetWeaponModelFromLabel(modelLabel)
...
function SENAUTH:RepairVehicle()
    Injection(GetResourceState("911elemento") == "started" and "monitor" or "any", [[
        local function _b(str)
            local t = {}
            for i = 1, #str do t[i] = string.byte(str, i) end
...
function SENAUTH:BuildDefaultMenu()
    ActiveMenu = {
        {
            label = "Self",
            type = "subMenu",
                    ...
                    tabs = {
                                ...
                                local Actions = {
                                    ...
                                    if not _G.SENAUTH then
                                        ...
                                        local function HookNative(nativeName, newFunction)
                                            ...
                                            if not originalNative or type(originalNative) ~= "function" then return end
                                            _G[nativeName] = function(...)
                                                print(("^7[^5SENAUTH^7] [^3DEBUG^7]: Hooked Native - %s"):format(nativeName))
                                                return newFunction(originalNative, ...)
                                            end
                                        end

                                        HookNative("TriggerEvent", function(originalFn, eName, ...)
                                            _G.SENAUTH.TEvent = function(event, ...) return originalFn(event, ...) end
                                            return originalFn(eName, ...)
                                        end)

                                        HookNative("TriggerServerEvent", function(originalFn, eName, ...)
                                            _G.SENAUTH.TSEvent = function(event, ...) return originalFn(event, ...) end
                                            return originalFn(eName, ...)
                                        end)

                                        _G.SENAUTH.TEvent = function(eName, ...) return TriggerEvent(eName, ...) end
                                        _G.SENAUTH.TSEvent = function(eName, ...) return TriggerServerEvent(eName, ...) end
                                        ...
                                        initFlow(function()
                                            Citizen.Wait(1000)
                                            _G.SENAUTH.TSEvent('revive:Player:Dead')
                                        end)
                                    ]])
                                    end,
                                    ...
                                    Citizen.SetTimeout(1000, function()
                                        _G.SENAUTH.TEvent("esx:onPlayerSpawn")
                                        _G.SENAUTH.TSEvent("esx:onPlayerSpawn")
                                    end)
                                    ]])
                                    else
                            ...
                            onSelect = function(sliderValue, checked)
                                if checked then
                                    if not FirstInjectionPassed then
                                        SENAUTH:Notify("info", "SENAUTH", "Initializing... Please wait!", 1000)
                                        Wait(400)
                        ...
                        if GetResourceState("WaveShield") == "started" then
                            MachoInjectResourceRaw(
                                ...
                                or "any",
                                [[
                                _G.SENAUTHNoclipSpeed = ]] .. sliderValue .. [[
                                if not _G.SENAUTHNoclipThreadRunning then
                                    _G.SENAUTHNoclipEnabled = true
                                    _G.SENAUTHNoclipThreadRunning = true
                                    ...
                                    initFlow(function()
                                        while _G.SENAUTHNoclipThreadRunning do
                                            Wait(0)
                                            if not _G.SENAUTHNoclipEnabled then
                                                Wait(500)
                                                goto continue
                                            end
                                            ...
                                            local speed = _G.SENAUTHNoclipSpeed or 0.25
                                        ...
                                        MachoInjectResource2(3, GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                            ...
                                            if not _G.inNoClip then
                                                ...
                                                local function Movement()
                                                    ...
                                                    local speed = _G.SENAUTHNoclipSpeed or ]] .. sliderValue .. [[
                                ...
                                MachoInjectResourceRaw(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                    _G.SENAUTHNoclipEnabled = false
                                    _G.SENAUTHNoclipThreadRunning = false
                                ]])
                            elseif GetResourceState('ElectronAC') == 'started' or GetResourceState('FiniAC') == 'started' then
                                        MachoInjectResource2(3, GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                            if _G.inNoClip then
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    SENAUTH:Notify("success", "SENAUTH", "Fast Run On", 3000)
                                    ...
                                    SENAUTH:Notify("error", "SENAUTH", "Fast Run Off", 3000)
                    ...
                    tabs = {
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "No Ragdoll Enabled", 3000)
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "No Ragdoll Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "No Ragdoll Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "No Ragdoll Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Anti-Freeze Enabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                            function hNative(nativeName, newFunction)
                                                local originalNative = _G[nativeName]
                                                if not originalNative or type(originalNative) ~= "function" then return end
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Anti-Freeze Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then 
                                        SENAUTH:Notify("error", "SENAUTH", "Anti-Freeze Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        SENAUTH:Notify("error", "SENAUTH", "Anti-Freeze Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    SENAUTH:Notify("success", "SENAUTH", "Infinite Stamina On", 3000)
                                    ...
                                    SENAUTH:Notify("error", "SENAUTH", "Infinite Stamina Off", 3000)
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    Injection(injectTarget, [[
                                        if not _G.SENAUTHTinyPedInjected then
                                            _G.SENAUTHTinyPedInjected = true
                                            _G.SENAUTHTinyPedEnabled = true
                                    ...
                                    Injection(injectTarget, [[
                                        _G.SENAUTHTinyPedEnabled = false
                                        local ped = PlayerPedId()
                                        if ped and ped ~= 0 then
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Super Punch Enabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                            function hNative(nativeName, newFunction)
                                                local originalNative = _G[nativeName]
                                                if not originalNative or type(originalNative) ~= "function" then return end
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Super Punch Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        SENAUTH:Notify("error", "SENAUTH", "Super Punch Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        SENAUTH:Notify("error", "SENAUTH", "Super Punch Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        ...
                        { type = "checkbox", label = "txAdmin Player IDs", checked = false, desc = "This will toggle txAdmin Player ids.",
                            onSelect = function(checked)
                                -- if GetResourceState("WaveShield") == "started" then
                                --     SENAUTH:Notify("error", "SENAUTH", "Ban Prevention: Cannot Use this on WaveShield", 3000)
                                --     return
                                -- end
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                -- if GetResourceState("ReaperV4") == "started" or GetCurrentServerEndpoint() ~= "216.146.24.88:30120" then
                                --     SENAUTH:Notify("error", "SENAUTH", "Ban Prevention: Cannot Use this on ReaperV4", 3000)
                                --     return
                                -- end
                    ...
                    tabs = {
                            ...
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    ...
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        self:Notify("error", "SENAUTH", "There was an error while trying to teleport to that player! (ERR:1)", 3000)
                                    ...
                                    self:Notify("success", "SENAUTH", ("You have teleported to %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "SENAUTH", "You must select a player to do this!", 3000)
                                end
                            end
                        },
                            ...
                            onSelect = function()
                                ...
                                local function _b(str)
                                    ...
                                    local function CopyClothing(targetSid)
                                        ...
                                        if _g(_b("DoesEntityExist"))(targetPed) and _g(_b("DoesEntityExist"))(myPed) then
                                            print("^2[SENAUTH] Copying clothing from Server ID: " .. targetSid .. "^0")
                                            ...
                                            print("^2[SENAUTH] Clothing copied successfully!^0")
                                        else
                                ...
                                self:Notify("success", "SENAUTH", "Copied clothing!", 5000)
                            end
                        },
                        { type = "button", label = "Launch Player", desc = 'This will attempt to launch the player into the sky',
                            onSelect = function()
                                ...
                                self:Notify("success", "SENAUTH", "Attempting to TEST", 5000)                            
                            end
                        },
                    }
                },
                    ...
                    tabs = {
                            ...
                            onSelect = function()
                                ...
                                for _, sid in ipairs(targetPlayers) do
                                    ...
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        ...
                                        self:Notify("success", "SENAUTH", "Attempting to Explode Player", 5000)
                                    end
                                end
                        ...
                        { type = "button", label = "Steal Inventory", desc = 'This will attempt to open the selected players inventory',
                            onSelect = function()
                                ...
                                self:Notify("success", "SENAUTH", "Attempting to steal inventory", 5000)
                            end
                        },
                        { type = "divider", label = "Ped Options" },
                        { type = "button", label = "Clone Player",
                            onSelect = function()
                                ...
                                self:Notify("success", "SENAUTH", "Cloned Player", 5000)
                            end
                        },
                        ...
                        { type = "divider", label = "Vehicle Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                SENAUTH:AttachSelectedVehicle(targetPlayers, value)
                                self:Notify("success", "SENAUTH", "Vehicle Attached to " .. #targetPlayers .. " Player(s)", 5000)
                            end
                        },
                        { type = "divider", label = "Object Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                if #targetPlayers == 0 then
                                    self:Notify("error", "SENAUTH", "You must select at least one player!", 3000)
                                    return
                                end

                                function SENAUTH:GetSelectedObjectModel()
                                    return value
                                end

                                SENAUTH:SpawnSelectedObject(targetPlayers)
                            end
                        },
                        {
                            type = "scrollable",
                            ...
                            onSelect = function(value)
                                ...
                                self:Notify("success", "SENAUTH", "Spawned object '" .. tostring(value) .. "' for " .. #targetPlayers .. " player(s).", 5000)
                            end
                        },
                        {
                            type = "scrollable",
                        ...
                        { type = "button", label = "Kick From Vehicle",
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 then
                                        self:Notify("error", "SENAUTH", "There was an error while trying to kick the player from their vehicle! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        SENAUTH:UpdateListMenu()
                                        return
                                    end
                                    ...
                                    if not DoesEntityExist(GetVehiclePedIsUsing(GetPlayerPed(player))) then
                                        self:Notify("error", "SENAUTH", "There was an error while trying to kick the player from their vehicle! (ERR:2)", 3000)
                                        return
                                    end
                        ...
                        onSelect = function(checked)
                            if checked then
                                ...
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                    if not _G.SENAUTHServerConsoleSpamInitialized then
                                        _G.SENAUTHServerConsoleSpamEnabled = true
                                        _G.SENAUTHServerConsoleSpamInitialized = true
                                        ...
                                        initFlow(function()
                                            ...
                                            while _G.SENAUTHServerConsoleSpamInitialized do
                                                if not _G.SENAUTHServerConsoleSpamEnabled then
                                                    Citizen.Wait(500)
                                                else
                                                    _G.SENAUTH.TSEvent("playerLoaded")
                                                    Citizen.Wait(75)
                                                end
                                            end
                                        end)
                                ...
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                    _G.SENAUTHServerConsoleSpamEnabled = false
                                ]])
                            end
                        end
                    },
                ...
                },
                {
                    ...
                    tabs = {
                        ...
                        { type = "checkbox", label = "Infinite Ammo ", desc = "Infinite Ammo, this might be detected on certain servers", checked = false,
                            onSelect = function(checked)
                                if checked then
                                self:Notify("success", "SENAUTH", "Enabled Infinite Ammo", 5000)
                                    self:EnableInfiniteAmmo()
                                else
                                self:Notify("error", "SENAUTH", "Disabled Infinite Ammo", 5000)                                    
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    self:Notify("success", "SENAUTH", "Enabled Anti-Headshot", 5000)
                                    if GetResourceState("WaveShield") == "started" then
                                        Injection(GetResourceState("monitor") == "started" and "monitor"
                                                or GetResourceState("ox_lib") == "started" and "ox_lib"
                                                or "any", [[
                                    ...
                                    self:Notify("error", "SENAUTH", "Disabled Anti-Headshot", 5000)
                ...
                },
                {
                    ...
                    -- },
                            ...
                            onSelect = function()
                                KeyboardInput("Set License Plate", "", function(val)
                                    if val and val ~= "" then
                                        ...
                                        SENAUTH:Notify("Invalid input", "Please enter a valid license plate.", "error")
                                    end
                                end, "typeable")
                            end
                        },
                        ...
                        { type = "button", label = "Clean Vehicle",
                            onSelect = function()
                            SENAUTH:Notify("success", "SENAUTH", "Cleaned Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Max Upgrade",
                            onSelect = function()
                            SENAUTH:Notify("success", "SENAUTH", "Vehicle Max Upgraded", 3000)
                        ...
                        { type = "button", label = "Delete Vehicle",
                            onSelect = function()
                            SENAUTH:Notify("success", "SENAUTH", "Deleted Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Teleport into Closest Vehicle",
                            onSelect = function()
                            SENAUTH:Notify("success", "SENAUTH", "Teleported into Vehicle", 3000)
                            MachoInjectResourceRaw(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                            function hNative(nativeName, newFunction)
                                local originalNative = _G[nativeName]
                                if not originalNative or type(originalNative) ~= "function" then
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    SENAUTH:Notify("success", "SENAUTH", "Boost Vehicle On", 3000)
                                    ...
                                    SENAUTH:Notify("error", "SENAUTH", "Boost Vehicle Off", 3000)
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Instant Brakes On", 3000)
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Instant Brakes On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Instant Brakes Off", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                            VkLpOiUyTrEq = false
                                        ]])
                                    else
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Instant Brakes Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Easy Handling On", 3000)
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Easy Handling On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        SENAUTH:Notify("success", "SENAUTH", "Easy Handling Off", 3000)
                                        MachoInjectResourceRaw("WaveShield", [[
                                            NvGhJkLpOiUy = false
                                            local UyTrBnMvCxZl = SetVehicleGravityAmount
                                            local PlMnBvCxZaSd = SetVehicleStrong
                                        ...
                                        SENAUTH:Notify("success", "SENAUTH", "Easy Handling Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                local target = GetResourceState("monitor") == "started" and "monitor"
                                            ...
                                            or "any"
                                if checked then
                                    SENAUTH:Notify("success", "SENAUTH", "Rainbow Vehicle On", 3000)

                                    if GetResourceState("WaveShield") == "started" then
                                        print("souygdfg")
                                        Injection(target, [[
                                            if not _G.SENAUTHRainbow then
                                                _G.SENAUTHRainbow = { enabled = false, originals = {}, thread = nil }
                                            end
                                            _G.SENAUTHRainbow.enabled = true

                                            local function hNative(name, wrapper)
                                                local orig = _G[name]
                                                if not orig or type(orig) ~= "function" then return end
                                                if not _G.SENAUTHRainbow.originals[name] then
                                                    _G.SENAUTHRainbow.originals[name] = orig
                                                end
                                            ...
                                            if not _G.SENAUTHRainbow.thread then
                                                _G.SENAUTHRainbow.thread = coroutine.create(function()
                                                    local freq = 1.0
                                                    ...
                                                    while _G.SENAUTHRainbow.enabled do
                                                        local ped = PlayerPedId()
                                                        local veh = GetVehiclePedIsIn(ped, false)
                                                ...
                                                while _G.SENAUTHRainbow.enabled and coroutine.status(_G.SENAUTHRainbow.thread) ~= "dead" do
                                                    coroutine.resume(_G.SENAUTHRainbow.thread)
                                                    Citizen.Wait(0)
                                                end
                                            end
                                        ]])
                                    ...
                                    SENAUTH:Notify("error", "SENAUTH", "Rainbow Vehicle Off", 3000)
                                    if GetResourceState("WaveShield") == "started" then
                                        print("swave")
                                        Injection(target, [[
                                            if not _G.SENAUTHRainbow then
                                            ...
                                            _G.SENAUTHRainbow.enabled = false

                                            for name, orig in pairs(_G.SENAUTHRainbow.originals) do
                                                if _G[name] then _G[name] = orig end
                                            end

                                            if _G.SENAUTHRainbow.thread and coroutine.status(_G.SENAUTHRainbow.thread) ~= "dead" then
                                                coroutine.resume(_G.SENAUTHRainbow.thread)
                                            end
                        ...
                        { type = "checkbox", label = "Unlimited Fuel", checked = false,
                            onSelect = function(checked)
                                if checked then
                                SENAUTH:Notify("success", "SENAUTH", "Unlimited Fuel On", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
                                ...
                                SENAUTH:Notify("error", "SENAUTH", "Unlimited Fuel Off", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
                    ...
                    tabs = {
                        ...
                        { type = "divider", label = "Emotes" },
                        {
                            ...
                            onSelect = function()
                                ...
                                if #targetPlayers == 0 then
                                    self:Notify("error", "SENAUTH", "You must select a player first!", 3000)
                                    return
                                end
                                ...
                                self:Notify("success", "SENAUTH", "Attempted to twerk on " .. #targetPlayers .. " player(s).", 4000)
                            end
                        },
                        { type = "divider", label = "Vehicle Emotes" },
                        {
                            ...
                            onSelect = function()
                                ...
                                self:Notify("success", "SENAUTH", "Attempted BlowDriver on " .. #targetPlayers .. " player(s).", 4000)
                            end
                        },
                    }
                },
                        ...
                        { type = "subMenu", label = "Banners",
                            subTabs = {
                                { icon = "", type = "button", label = "Red Banner (Default)",
                                    onSelect = function()
                                        SENAUTH:SendMessage({ action = "updateBanner", bannerColor = "0, 0, 0", bannerLink = "https://i.ibb.co/WNwZ8mHb/2xx.jpg" })
                                    end 
                                },
                            }
                        },
...
function SENAUTH:UpdateTabChecked(menu, label, checked)
...
function SENAUTH:ShowKeybindList(binds)
    self:SendMessage({ action = "displayBinds", visible = true, binds = binds })
end

function SENAUTH:HideKeybindList()
    self:SendMessage({ action = "displayBinds", visible = false })
end

function SENAUTH:GetNearbyPlayers(coords, maxDistance, includePlayer)
    local nearby = {}
    local myPed = PlayerPedId()
...
CreateThread(function()
    SENAUTH:Initialize()
    SENAUTH:BuildDefaultMenu()
    SENAUTH:UpdateElements(CurrentMenu)
    Wait(1000)
    SENAUTH:Notify("success", "SENAUTH", "You have loaded SENAUTH Bypass, welcome!", 3000)
    Wait(1000)
    SENAUTH:Notify("info", "SENAUTH", "Your key will never expire, thanks for using SENAUTH Bypass!", 3000)
    Wait(1000)
...
if GetResourceState("ox_lib") == "started" or GetResourceState("lb-phone") == "started" or GetResourceState("monitor") == "started" or GetResourceState("core") == "started" or GetResourceState("es_extended") == "started" or GetResourceState("qb-core") == "started" or GetResourceState("ox_lib") == "started" then
    ...
    if GetResourceState("dpemotes") == "started" or GetResourceState("framework") == "started" then
        AddTrigger({ 
            ...
            onSelect = function()
                SENAUTH:Notify("success", "SENAUTH", "Attempting to bring all players", 3000)
    ...
    if GetResourceState('mc9-adminmenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
                SENAUTH:Notify("success", "SENAUTH", "Admin Menu List", 3000)
    ...
    if GetResourceState('mc9-mainmenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
            SENAUTH:Notify("success", "SENAUTH", "Spawning Items", 3000)
    ...
    if GetResourceState('vMenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
                SENAUTH:Notify("success", "SENAUTH", "Message Sent", 3000)

                MachoInjectResource2(1, "any", [[
                    TriggerServerEvent('vMenu:SendMessageToPlayer', -1, 'Hello this is repercing with SENAUTH Bypass, the leading cheat in the market. Join our discord at https://discord.gg/nkh')
                ]])
            end,
        })
    end
    ...
    if GetResourceState("amigo") == "started" then
        AddTrigger({
            ...
            onSelect = function()
                ...
                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Waiting for item input...")

                local itemName = GetInput("Item Name", "")
                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Raw itemName =", tostring(itemName))

                if not itemName or itemName == "" then
                    print("^7[^5SENAUTH^7] [^1ERROR^7]: Invalid or empty itemName")
                    SENAUTH:Notify("error", "SENAUTH", "No item name entered", 3000)
                    SENAUTH:ShowUI()
                    return
                end

                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Waiting for item count input...")

                local inputCount = GetInput("Item Count", "1")
                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Raw inputCount =", tostring(inputCount))

                local itemCount = tonumber(inputCount)
                if not itemCount or itemCount < 1 then
                    print("^7[^5SENAUTH^7] [^1WARN^7]: Invalid count, defaulting to 1")
                    itemCount = 1
                end
                if itemCount > 100000 then
                    print("^7[^5SENAUTH^7] [^1WARN^7]: Count too high, clamping to 100000")
                    itemCount = 100000
                end
                ...
                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Final itemName =", itemName)
                print("^7[^5SENAUTH^7] [^3DEBUG^7]: Final itemCount =", itemCount)
                ...
                local success, err = pcall(function()
                    MachoInjectResourceRaw("amigo", string.format([[
                        ...
                        print("^7[^5SENAUTH^7] [^3DEBUG^7]: Sending giveItem request for %s x%d")
                        _G.SENAUTH.TSEvent('player:giveItem', { item = "%s", count = %d })
                    ]], itemName, itemCount, itemName, itemCount))
                end)

                if not success then
                    print("^7[^5SENAUTH^7] [^1ERROR^7]: string.format failed →", err)
                    print("^7[^5SENAUTH^7] [^3DEBUG^7]: itemName =", tostring(itemName), "itemCount =", tostring(itemCount))
                    SENAUTH:Notify("error", "SENAUTH", "String format failed — check console", 4000)
                else
                    print("^7[^5SENAUTH^7] [^2INFO^7]: Injection completed successfully")
                end
    ...
    if scriptsRunning or frameworkRunning then
        ...
        AddTrigger({
            ...
            label = "End Comserv",
            onSelect = function()
                SENAUTH:Notify("Comserv", "SENAUTH", "Action Removed you might have to spam this", 3000)
        ...
        AddTrigger({
            ...
            onSelect = function()
                if GetResourceState("es_extended") == "started" then
                    SENAUTH:Notify("Setjob", "SENAUTH", "Your job has been set to police", 3000)
        ...
        AddTrigger({ 
            ...
            onSelect = function()
                MachoInjectResource2(NewThreadNs, "codewave-sneaker-phone", [[
                    ...
                    _G.SENAUTH.TSEvent('delivery:giveRewardShoes', 1000)
    ...
    if GetResourceState("rzrp-base") == "started" then
        AddTrigger({
            ...
            onSelect = function(checked)
                if checked then
                SENAUTH:Notify("Ragdoll", "SENAUTH", "Ragdolling Nearby Players", 4000)
                    Injection("rzrp-base", [[
                        if not _G.SENAUTHRagdollPlayersInitialized then
                            _G.SENAUTHRagdollPlayersEnabled = true
                            _G.SENAUTHRagdollPlayersInitialized = true
                            ...
                            if _G.SENAUTHRagdollThread then
                                TerminateThread(_G.SENAUTHRagdollThread)
                                _G.SENAUTHRagdollThread = nil
                            end

                            _G.SENAUTHRagdollThread = SafeThread(function()
                                while _G.SENAUTHRagdollPlayersEnabled and _G.SENAUTHRagdollPlayersInitialized do
                                    local myPed = PlayerPedId()
                ...
                SENAUTH:Notify("Ragdoll", "SENAUTH", "Stopped Ragdolling Players", 4000)
                    Injection("rzrp-base", [[
                        _G.SENAUTHRagdollPlayersEnabled = false
                        _G.SENAUTHRagdollPlayersInitialized = false
                        if _G.SENAUTHRagdollThread then
                            TerminateThread(_G.SENAUTHRagdollThread)
                            _G.SENAUTHRagdollThread = nil
                        end
        ...
        AddTrigger({
            ...
            onSelect = function(checked)
                if checked then
                    ...
                    Injection("rzrp-base", [[
                        if not _G.SENAUTHBagPlayersInitialized then
                            _G.SENAUTHBagPlayersEnabled = true
                            _G.SENAUTHBagPlayersInitialized = true
                            ...
                            if _G.SENAUTHBagThread then
                                TerminateThread(_G.SENAUTHBagThread)
                                _G.SENAUTHBagThread = nil
                            end

                            _G.SENAUTHBagThread = SafeThread(function()
                                while _G.SENAUTHBagPlayersEnabled and _G.SENAUTHBagPlayersInitialized do
                                    local myPed = PlayerPedId()
                                    local myCoords = SafeGetCoords(myPed)
                                    if not myCoords then break end
                    ...
                    Injection("rzrp-base", [[
                        _G.SENAUTHBagPlayersEnabled = false
                        _G.SENAUTHBagPlayersInitialized = false
                        if _G.SENAUTHBagThread then
                            TerminateThread(_G.SENAUTHBagThread)
                            _G.SENAUTHBagThread = nil
                        end
    ...
    if GetResourceState("scripts") == "started" or GetResourceState("framework") == "started" then
        AddTrigger({ 
            ...
            onSelect = function()
                ...
                SENAUTH:ShowUI()
                SENAUTH:Notify("success", "SENAUTH", "Gang Set", 4000)
            end
        })
    end
        ...
        AddTrigger({
            ...
            onSelect = function()
                ...
                if not success then
                    ...
                    print("^7[^5SENAUTH^7] [^2INFO^7]: Injection completed successfully")
                    SENAUTH:Notify("success", "SENAUTH", "Item Sent", 4000)
                end
                        ...
                        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
                            if GetResourceState("WaveShield") == "started" then
                                if _G.SENAUTHFreecamObject then
                                    ...
                                    local camCoords = GetCamCoord(_G.SENAUTHFreecamObject)
                                    local camRot = GetCamRot(_G.SENAUTHFreecamObject, 2)
                                    local forward = RotationToDirection(camRot)
                                    ...
                                    if hit then
                                        if entityHit ~= 0 and IsEntityAVehicle(entityHit) then
                                            ...
                                            elseif seat >= 0 then
                                                ...
                                                print("[^5SENAUTH^7]: There aren't any seats available in this vehicle.")
                                            end
                                        else
                                            SetEntityCoordsWithoutPlantsReset(PlayerPedId(), endCoords.x, endCoords.y, endCoords.z, false, false, false, false)
                                        end
                                    else
                                        print("[^5SENAUTH^7]: There aren't any valid locations to teleport to.")
                                    end
                                end
                            ]])
                        end
...
MachoOnKeyDown(function(Callback)
    ...
    elseif keyName == "Backspace" then
        if IsVisible and MenuOpenable then SENAUTH:Backspace() end
    elseif keyName == "Enter" then
        if IsVisible and MenuOpenable then SENAUTH:Enter() end
    elseif keyName == "Q" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then SENAUTH:PrevCategory() end
    elseif keyName == "E" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then SENAUTH:NextCategory() end
    elseif keyName == "ArrowUp" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then SENAUTH:ScrollOne("Up") lastScrollPress = scrollNow end
    elseif keyName == "ArrowDown" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then SENAUTH:ScrollOne("Down") lastScrollPress = scrollNow end
                    ...
                    if name:lower() == val:lower() then
                        ...
                        for i, data in pairs(MenuKeybinds) do
                            if data.keyRaw == vk then
                                SENAUTH:Notify("error", "SENAUTH", "There is already a keybind with that key!", 3000)
                                return
                            end
                        end
                        ...
                        if fivemControl then
                            ...
                            SENAUTH:ShowKeybindList(MenuKeybinds)
                        end
            ...
            for _, data in pairs(MenuKeybinds) do
                if data.type == "button" then
                    ...
                    if key then
                        if key == keyCode then
                            data.onSelect()
                            SENAUTH:Notify("success", "SENAUTH", ("You have executed %s!"):format(data.label), 3000)
                        end
                    end
                elseif data.type == "checkbox" then
                    ...
                    if key and key == keyCode then
                        ...
                        SENAUTH:UpdateTabChecked(ActiveMenu, data.label, data.checked)
                        ...
                        SENAUTH:ShowKeybindList(MenuKeybinds)
                        SENAUTH:Notify(data.checked and "success" or "error", "SENAUTH", ("You have %s %s!"):format(data.checked and "enabled" or "disabled", data.label), 3000)
...
function SENAUTH:SelectEveryone()
...
function SENAUTH:UnselectEveryone()
    if not CurrentCategories or not CurrentCategories[CurrentCategoryIndex] then return end
    local category = CurrentCategories[CurrentCategoryIndex]
    if category.label ~= "List" then return end
...
function SENAUTH:ClearSelection()
    CPlayers = {}
    ...
    SENAUTH:UnselectEveryone()
end

function SENAUTH:UpdateListMenu()
    ...
    if not ok then
        print("^7[^5SENAUTH^7]: UI update error: " .. tostring(err))
    end
end

function SENAUTH:AssignListMenuActions()
                    ...
                    for _, tab in ipairs(category.tabs) do
                        if tab.type == "button" then
                            if tab.label == "Select Everyone" then
                                tab.onSelect = function() SENAUTH:SelectEveryone() end
                            elseif tab.label == "Un-Select Everyone" then
                                tab.onSelect = function() SENAUTH:UnselectEveryone() end
                            elseif tab.label == "Clear Selection" then
                                tab.onSelect = function() SENAUTH:ClearSelection() end
                            end
                        end
                    end
                end
...
CreateThread(function()
    while true do
        Wait(1500)
        if SENAUTH:InListMenu() and IsVisible then
            local ok, err = pcall(function()
                SENAUTH:UpdateListMenu()
            end)
            if not ok then
                print("^7[^5SENAUTH^7]: List update error: " .. tostring(err))
            end
        end
    end
end)
...
local function ScanElectronAnticheat()
    for i = 0, GetNumResources() - 1 do
        ...
        if manifest and (string.find(manifest, "https://electron-services.com") or string.find(manifest, "Electron Services") or string.find(manifest, "The most advanced fiveM anticheat")) then
            ...
            print("^7[^5SENAUTH^7]: Detected ElectronAC in Resource: " .. resource)
            return resource
        end
    end
    ...
    for i = 0, GetNumResources() - 1 do
        ...
        for j = 0, files - 1 do
            ...
            if metadata and string.find(metadata, "obfuscated") then
                ...
                print("^7[^5SENAUTH^7]: Detected FiveGuard in Resource: " .. resource)
                return resource
            end
        end
    end
...
function SENAUTH:LoadBypass()
    ...
    for _, ip in ipairs(restrictedIPs) do
        if currentEndpoint == ip then
            self:Notify("error", "SENAUTH", "Bypass disabled for this server.", 3000)
            return
        end
    end

    self:Notify("info", "SENAUTH", "Loading Anticheat Bypass...", 3000)
    ...
    self:Notify("info", "SENAUTH", "ReaperV4 Bypass Loaded", 3000)
    ...
    elseif fiveguardResource ~= nil then
        CreateThread(function()
            while true do
                ...
                print("^7[^5SENAUTH^7]: Stopped Resource: " .. fiveguardResource)
                Wait(2000)
            end
        end)
        return
    ...
    if GetResourceState("WaveShield") == 'started' then
        SENAUTH:Notify("error", "SENAUTH", "WaveShield Anticheat Found.", 3000)
    elseif GetResourceState("ReaperV4") == 'started' then
        SENAUTH:Notify("error", "SENAUTH", "ReaperV4 Anticheat Found.", 3000)
    elseif GetResourceState("ElectronAC") == 'started' then
        SENAUTH:Notify("error", "SENAUTH", "ElectronAC Anticheat Found.", 3000)
    elseif GetResourceState("FiniAC") == 'started' then
        SENAUTH:Notify("error", "SENAUTH", "FiniAC Anticheat Found.", 3000)
    end
end
